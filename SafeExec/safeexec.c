 ...
 *
 * Idea:
 *   we use stdin and stdout to pass and receive information to
...
#define XLARGECONST                                          \
  2 * LARGECONST  /* recent version of Java need more memory \
                     */
#define SIZE 8192 /* buffer size for reading /proc/<pid>/status */
#define INTERVAL                                     \
  67 /* about 15 times a second                    * \
...
int microseconds(struct timeval* tv) {
  return ((int)tv->tv_sec * 1000000 + (int)tv->tv_usec / 1);
}
          ...
          if (WIFSIGNALED(status) != 0) {
            ...
            if (WTERMSIG(status) == SIGKILL)
              ...
              printstats("Command terminated by signal (%d: %s)\n",
                         WTERMSIG(status), name(WTERMSIG(status)));
